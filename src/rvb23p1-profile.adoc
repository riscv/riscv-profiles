...
Only supervisor-mode (RVB23S64) profiles are
...
built on top of the extensions lised in the RVB23 profiles.


=== RVB23.1S64 Profile

The RVB23.1S64 profile specifies the ISA features available to a
supervisor-mode execution environment in 64-bit applications
processors.  RVB23.1S64 is based on and requires an RVB23S64
implementation. Being a minor release, this only 

==== RVB23S64 Optional Extensions

RVB23.1S64 has the same unprivileged options as RVB23U64,

The privileged options in RVB23.1S64 are listed in the following
