...
The following mandatory extension is RVY (CHERI) specific:

- *Zys* Sentry definition
- *Zabhlrsc* Byte and half-word load-reserved and store conditional

The following extension is incompatible with RVY:
...
The following localized options are taken from RVA23:
...
The following are new development options intended to become mandatory in a future RVYA profile.
...
The following are new expansion options in RVYA25U64:
...
There are no transitory options in RVYA25U64.

NOTE: Scalar crypto is no longer an option in RVYA25U64, though the Zbc
...
There are no privileged development options in RVYA25S64.
...
The following are new privileged expansion options in RVYA25S64
...
claiming to be RVYA25-compatible, which must have the hypervisor
...
environment). Ssstrict (and all the other RVYA25 mandates and options)
do not apply to any guest VMs run by a hypervisor. An RVYA25 hypervisor
can provide guest VMs that are also RVYA25-compatible but with an
expanded set of emulated standard instructions. An RVYA25 hypervisor
can also choose to implement guest VMs that are not RVYA25 compatible
...
There are no privileged transitory options in RVYA25S64.
