...
The following mandatory extension is RVY specific and specifies the capability encoding format:

- *Zydefaultcap* Default capability encoding (visible to software as uycfg.base=0).

The following mandatory extensions are RVY specific, but may be replaced over time with binary compatible alternatives if better schemes are developed:

- *Zys* Sentry definition
- *Zylevels1* 2-level Capability Information Flow Control (visible to software as uycfg.features=1).
...
The following development option from RVA23U645 is mandatory in RVYA25U64, as it is naturally paired with Zabhlrsc:
...
- *Zicfilp* Landing Pads
 ...
 NOTE: Zicfiss Shadow Stack is currently incompatible with RVY, an RVY version may be available in the future.
...
The following extension RVY specific, is mandatory in RVYA25S64, but may be replaced over time by a binary compatible alternative if a better scheme is developed:
...
- *Zyhybrid for privileged architectures* Support for enabling CHERI at different privilege levels (implies, and implied by Zyhybrid)

NOTE: Implementing *Zyhybrid for privileged architectures* requires that the implementation supports the full RVA23S64 profile.
...
The following unprivileged A extensions are RVY specific:

- *Zys* Sentry definition
- *Zyhybrid* Support for RV64I base architecture

The following unprivileged ISA extension is general-purpose but added for CHERI software:
...
The following privileged ISA extensions are RVY specific:
