...
The RVI20 profiles are only intended to be targets for application
code, and so RVI20S and RVI20M profiles are not defined.
...
- D
- C
- stvec writable, direct+vectored modes, ...
- scounteren
...
- User mode (adds mstatus.MPP, mstatus.MPRV, ...)
- Supervisor mode (Sbare only)
- PMPs
- mstatus.TW (timeout wait - or make mandatory?)
- mstatus.TSR (Trap SRET - or make unsupported?)
