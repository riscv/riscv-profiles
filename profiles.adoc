...
mode.  For this reason, there are separate profiles for unprivileged
code and for each privileged mode that a profile family supports.

Unprivileged profiles include only unprivileged ISA features.
