...
combinatorial explosion in possible ISA choices.  Many applications
depend on a rich software ecosystem, which is only made possible by
reducing the possible combinations of ISA features that the software
ecosystem has to support.
...
Architecture profiles provide a structure for describing these
commonly used combinations of instruction set extensions within broad
application domains.  Each profile is built on a standard base ISA
plus a set of mandatory ISA extensions, and provides a small set of
standard ISA options to extend the mandatory set.  Profiles provide a
convenient shorthand for describing the ISA portions of hardware and
software platforms, and also guide the development of common software
...
specialized applications albeit without the expectation of widespread
software support.
...
platform (e.g., boot process, behavior of environment calls, presence
of certain memory-mapped hardware devices, etc.).  Architecture
profiles factor out ISA-specific definitions from platform definitions
to allow ISA profiles to be reused across different platforms, and to
be used by tools (e.g., compilers) that are common across many
different platforms.
...
profile.  A later release of a profile family may choose to include
the feature as Mandatory or Supported Optional.
...
RISC-V is a layered architecture supporting multiple privilege modes.
...
NOTE: Running unprivileged code is different from running code in
user-mode.  Unprivileged code is code that does not use any privileged
ISA features and can be run in any privilege mode.  Privileged code
using privileged ISA features can be run in a user-mode execution
environment but will normally cause traps into the enclosing execution
environment when privileged instructions are executed (this behavior
might be exploited, for example, to emulate a privileged profile in a
user-mode execution environment).

The profile for a privilege mode describes the ISA features of an
execution environment that has the named privilege mode as the
most-privileged mode available, but also includes all supported
lower-privilege modes.  For example, an S-mode profile includes the
behavior of instructions when running in different modes in an S-mode
execution environment, such as how an `ecall` instruction in U-mode
causes a contained trap into an S-mode handler, or how S-mode code can
set `sstatus.spp` to cause the processor to switch into U-mode after
executing an `sret` instruction.
...
up and including pure software emulation.
...
The RVA20U64 profile represents the behavior of user-mode execution
environments in 64-bit applications processors.  This is the most
important profile within the application processor family in terms of
the amount of software that targets this profile.
...
The RVA20S64 profile describes the ISA features of a supervisor-mode
execution environment based on privileged architecture version 1.11.
This is the profile used by rich operating system platforms.
...
- stvec.MODE must be capable of holding the value 0 (Direct).
  When stvec.MODE=Direct, stvec.BASE must be capable of holding any valid four-byte-aligned address.
...
NOTE: This is not generally used by rich operating systems.
