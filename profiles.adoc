...
expected to be able to support these extensions.  While individual
unsupported extensions may be a component of a supported option,
software is not expected to cope with separately enabling the
unsupported individual extensions, and may (but are not required to)
error or report warnings if they are present but not part of a
complete supported option.
...
later release of a profile may choose to include the new extension as
